...
def define_G_enc(input_nc, output_nc, ngf, which_model_netG, norm='batch', init_type='normal', init_gain=0.02, n_downsampling=2):
    netG = None
    norm_layer = get_norm_layer(norm_type=norm)
...
def define_G_core(input_nc, output_nc, ngf, which_model_netG, norm='batch', use_naive=False, 
                  init_type='normal', init_gain=0.02,invertible=False, n_downsampling=2, coupling='additive'):
    netG = None
    norm_layer = get_norm_layer(norm_type=norm)
...
class ResnetGenerator_core(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_naive=False, n_blocks=6,
                 padding_type='reflect', invertible=True, n_downsampling=2, coupling='additive'):
        assert(n_blocks >= 0)
...
class ResnetGenerator_dec(nn.Module):
   ...
   #########################################################################        
            # 假设输入张量为input_tensor
            # input_channels = 1
            # desired_output_channels = 256

            # # 使用 nn.Conv2d 进行通道增加
            # conv1 = nn.Conv2d(input_channels, desired_output_channels, kernel_size=1)
##########################################################################################################
