...
from modelsClassical.CQCC_wganRes import ClassicalGAN1
from modelsClassical.CQCC_wganRes2 import ClassicalGAN2
...
def train(dataset_str, batch_size,  checkpoint):
    ...
    out_dir = r"E:\Paper\Data\TrainResult\ClasscisalTrain\Wcyclegan\Noise\shuffle_Restruct_LamAdver8_Cycle40_FSIM300_Noise_3_cyclegan\result_A2B"
    out_dir_2 =r"E:\Paper\Data\TrainResult\ClasscisalTrain\Wcyclegan\Noise\shuffle_Restruct_LamAdver8_Cycle40_FSIM300_Noise_3_cyclegan\result_B2A"
    out_dir_3 =r"E:\Paper\Data\TrainResult\ClasscisalTrain\Wcyclegan\Noise\shuffle_Restruct_LamAdver8_Cycle40_FSIM300_Noise_3_cyclegan\result_A_real"
    out_dir_4 =r"E:\Paper\Data\TrainResult\ClasscisalTrain\Wcyclegan\Noise\shuffle_Restruct_LamAdver8_Cycle40_FSIM300_Noise_3_cyclegan\result_B_real"
...
########################################################
    ...
    # 计算生成器的参数数量
    total_params = sum(p.numel() for p in generator_2.parameters() if p.requires_grad)
    ...
    # data = ImageDataset(dataset_str)
    # a=len(data)
    # print(len(data))  # 打印数据集的长度

    optimizer_G = torch.optim.Adam(itertools.chain(generator.parameters(), generator_2.parameters()), lr=lr_G,
                                   betas=(b1, b2))
    ...
    for epoch in trange(n_epochs):
        for i, batch in enumerate(dataloader):
            ...
            if i % n_critic == 0:
                ...
                recover_A = generator_2(fake_images_B)
                g_loss_cycle_1 = criterion_cycle(real_A, recover_A) * 20

                #  # 计算fsim_value损失
                # fsim_value_1 = fsim(denorm(real_A), denorm(recover_A)) * 300
                # 计算总损失
                ...
                ######################生成器cycleABA的损失#################################
                recover_B = generator(fake_images_A)

                g_loss_cycle_2 = criterion_cycle(real_B, recover_B) *20
                #  # 计算fsim_value损失
                # fsim_value_2 = fsim(denorm(real_B), denorm(recover_B)) * 300
                # 计算总损失
                gloss_b2a = g_loss_2 + g_loss_cycle_2 
                loss2_history.append(gloss_b2a.item())
                # g_loss_sum = g_loss_1 + g_loss_2+ g_loss_cycle_1 + g_loss_cycle_2
                g_loss_sum = gloss_a2b + gloss_b2a
...
if __name__ == "__main__":
    #  train(r"E:\Acdamic\Data\DataMake\MySelfData\Dilate6_shuffle", 10, 0)
    train(r"E:\Acdamic\Data\DataMake\MySelfData\Noise\Noise_3_shuffle", 10, 0)
